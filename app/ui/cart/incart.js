...
import Allow from '../properies/allow';
import axios from 'axios';

function InCart() {

  const { push } = useRouter()
  const { totalIncart, carts, count, increment, decrement,handleRemove } = useContext(ServiceContext)
      ...
      <div className="containerCart mx-auto mt-10">
        <div className="sm:flex  my-10 px-4">
                        ...
                        <div className="md:pl-3 md:w-8/12 2xl:w-3/4 flex flex-col justify-center">
                          <div className="flex items-center justify-between w-full">
                            ...
                            <div className="flex items-center border-gray-100 text-gray-700 mt-3">
                              {/* <span onClick={()=>decrement(service)} className="cursor-pointer rounded-l bg-gray-200 py-1 px-3.5 duration-100 hover:bg-blue-500 hover:text-blue-50"  > - </span> */}
                              <p className="" > {service.day} days</p>
                              {/* <span onClick={()=>increment(service)} className="cursor-pointer rounded-r bg-gray-200 py-1 px-3 duration-100 hover:bg-blue-500 hover:text-blue-50" > + </span> */}
                            </div>
                          </div>
                          <div className='flex justify-between'>
                            <div>
                              <p className="text-xs leading-3 text-gray-600 pt-2">Allowed from: {service.start} </p>
                              <p className="text-xs leading-3 text-gray-600 py-4">To: {service.end}</p>
                            </div>
                            <div className='flex items-center'>
                              <button onClick={setShowAllow} className='bg-yellow-500 hover:bg-yellow-600 text-gray-100 p-2 rounded shadow'>Change date</button>
                            </div>
                          </div>
                          <div className="flex items-center justify-between pt-5">
                            <div className="flex itemms-center justify-center space-x-4">
                              ...
                              <svg onClick={() => handleRemove(service.id)} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="remove text-gray-600 h-6 w-6 cursor-pointer duration-150 hover:text-red-500" title="remove">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                              </svg>
                            </div>
                            <p className="text-base font-black leading-none text-gray-800"><PriceFormat myNumber={service.price} /> /Day</p>
                          </div>
                        </div>
                      </div>
          ...
          <p className="text-gray-700">120 000 000 000 BIF</p>
            ...
            <button onClick={handleCheckout} disabled={carts.length === 0} className="mt-6 w-full rounded-md bg-yellow-500 py-1.5 font-medium text-blue-50 hover:bg-yellow-600 hover:-translate-y-1 duration-500 transition-all">Check out</button>
          </div>
        </div>
      </div>
    </>
