    ...
    <>
      <div className="relative flex  flex-col overflow-hidden bg-white py-6 sm:py-12">
        <div className="  px-4 w-full mb-8">
          <h2 className="mb-4 font-bold text-[2rem] sm:text-[3rem]  md:text-[3.5rem] text-gray-800 ">
            Properties for you
          </h2>
          {services && (
            <div className="grid w-full sm:grid-cols-2 lg:grid-cols-3  gap-6 ">
              {services.map((service, index) => {
                return (
                  <div
                    key={index}
                    className="item1 relative flex flex-col shadow-md rounded-md overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300 max-w-md  h-[20rem]"
                  >
                    <button
                      type="button"
                      className="hover:text-yellow-600 absolute z-30 top-2 right-0 mt-2 mr-3"
                    >
                      <svg
                    ...
                    <Link
                      href={`/properties/${service.uuid}/detail`}
